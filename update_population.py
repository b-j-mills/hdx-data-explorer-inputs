            ...
            for header in headers:
                if not pcode_header:
                    ...
                    if codematch and levelmatch:
                        pcode_header = header
                popmatch = bool(re.search("(population|both|total|proj|pop|t|ensemble)", header, re.IGNORECASE))
                sexyearmatch = bool(re.search("_f|_m|m_|f_|year|female|male", header, re.IGNORECASE))
                agematch = bool(re.search("^\d{1,2}\D|(\D\d{1,2}\D)|(\D\d$)", header, re.IGNORECASE))
                agewordmatch = bool(re.search("(age|adult)", header, re.IGNORECASE))
                yearmatch = len(re.findall("\d{4}", header, re.IGNORECASE))
                if popmatch and not sexyearmatch and not agematch and not agewordmatch and yearmatch < 2:
                    if not pop_header:
                        pop_header = header
                    elif yearmatch > 0:
                        try:
                            ...
                            logger.info(f"Not sure which header to pick: {pop_header}, {header}")
                            continue
